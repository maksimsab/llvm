...
; -- Per-source split
; rUN: sycl-module-split -split=source -emit-param-info -emit-exported-symbols -split-esimd -lower-esimd -O2 -spec-const=native -S < %s -o %tA.table
; rUN: FileCheck %s -input-file=%tA_0.ll --check-prefixes CHECK-A0
; rUN: FileCheck %s -input-file=%tA_1.ll --check-prefixes CHECK-A1
; -- No split
; rUN: sycl-module-split -emit-param-info -emit-exported-symbols -split-esimd -lower-esimd -O2 -spec-const=native -S < %s -o %tB.table
; rUN: FileCheck %s -input-file=%tB_0.ll --check-prefixes CHECK-B0
; -- Per-kernel split
; rUN: sycl-module-split -split=kernel -emit-param-info -emit-exported-symbols -split-esimd -lower-esimd -O2 -spec-const=native -S < %s -o %tC.table
